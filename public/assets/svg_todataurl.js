...
function exportSVG(_svg) {
    let svg_xml = XMLSerialize(_svg);
    let svg_dataurl = base64dataURLencode(svg_xml);
...
function XMLSerialize(svg) {
    ...
    function XMLSerializerForIE(s) {
        ...
        out += "<" + s.nodeName;
        for (const element of s.attributes) {
        ...
        if (s.hasChildNodes()) {
            ...
            for (const element of s.childNodes) {
                out += XMLSerializerForIE(element);
            }
...
SVGElement.prototype.toDataURL = function (type, options) {
    ...
    function exportImage(type) {
        ...
        svg_img.onload = function () {
            ...
            let png_dataurl = canvas.toDataURL(type);
    ...
    function exportImageCanvg(type) {
        ...
        let keepBB = options.keepOutsideViewport;
        let bb = keepBB ? _svg.getBBox() : undefined;
        ...
        canvg(canvas, svg_xml, {
            ...
            renderCallback: function () {
                debug(
                    "exported image dimensions " + [canvas.width, canvas.height]
                );
                let png_dataurl = canvas.toDataURL(type);
                debug(type + " length: " + png_dataurl.length);
    ...
    if (options.keepOutsideViewport)
        debug(
            "NOTE: keepOutsideViewport is only supported with canvg exporter."
        );

    switch (type) {
        case "image/svg+xml":
            return exportSVG(_svg);
        ...
        case "image/jpeg":
            ...
            switch (options.renderer) {
                ...
                case "native":
                    debug(
                        "using native renderer for png export. THIS MIGHT FAIL."
                    );
